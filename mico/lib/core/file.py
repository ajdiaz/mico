...
def file_attribs(location, mode=None, owner=None, group=None, recursive=False):
    ...
    if group:
        _x = run('chgrp %s %s "%s"' % (recursive, group, location))[0]
...
def file_read(location):
    ...
    with fabric.context_managers.settings(fabric.api.hide('stdout')):
        _exe = run('openssl base64 -in "%s"' % (location))[0]
...
def file_attribs_get(location):
    ...
    if file_exists(location):
        fs_check = run('stat %s %s' % (location, '--format="%a %U %G"'))[0]
...
def file_md5(location):
    ...
    .. note::
        ...
        be on the safe side.
    """
    sig = run('md5sum "%s" | cut -d" " -f1' % (location))[0]
...
def file_update(location, updater=lambda x:x):
    ...
    _x = run("echo '%s' | openssl base64 -A -d -out '%s'" % (base64.b64encode(new_content), location))[0]
...
def file_append(location, content, mode=None, owner=None, group=None):
    ...
    """
    run('echo "%s" | openssl base64 -A -d >> "%s"' % (base64.b64encode(content), location))[0]
