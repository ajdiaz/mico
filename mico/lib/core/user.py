...
def user_password(name, password, encrypted_password=False):
    ...
    if encrypted_password:
        _x = execute("usermod -p '%s' '%s'" % (password,name))
    else:
        _x = execute("echo '%s' | openssl base64 -A -d | chpasswd" % (encoded_password))
...
def user_create(name, password=None, home=None, uid=None, gid=None, shell=None,
    ...
    _x = execute("useradd %s '%s'" % (" ".join(options), name))
    if password:
...
def user_exists(name=None, uid=None):
    ...
    if name != None:
        d = execute("cat /etc/passwd | egrep '^%s:'" % (name), force=True)
    else:
        d = execute("cat /etc/passwd | egrep '^.*:.*:%s:'" % (uid), force=True)
...
def user_remove(name, rm_home=False):
    ...
    """
    return execute("userdel -f %s '%s'" % (rm_home and "-r" or "", name))
